...
$(function(){
    ...
    $('.submit-schmooze-form').click(function(e){

        e.preventDefault();
        var that = $(this);
        postSchmoozeSignup(that);
    });
});
...
function postSchmoozeSignup(that) {

    console.log("Clicked");

    var email = that.siblings('input').val();
    ...
    if (!isValidEmailAddress(email)) {
        ...
        that.parent().siblings('.schmooze-confirmation').fadeIn(500);

        $.ajax({

            url: "https://docs.google.com/forms/d/1RqWxmzZMRJTWUYHo16aKGxXpOQtYJN1N-2k6oawG9MI/formResponse",
            data: {

                "entry.1917413184" : email,
            },
            type: "POST",
            dataType: "xml",
            async: false,
